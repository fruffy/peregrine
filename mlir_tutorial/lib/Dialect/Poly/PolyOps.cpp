    ...
    namespace tutorial
    {
        namespace poly
        {
            ...
            OpFoldResult MulOp::fold(MulOp::FoldAdaptor adaptor)
            {
                ...
                return DenseIntElementsAttr::get(RankedTensorType::get(static_cast<int64_t>(result.size()),
            ...
            LogicalResult EvalOp::verify()
            {
                ...
                auto isSignlessInteger = pointTy.isSignlessInteger(32);
                auto complexPt = llvm::dyn_cast<ComplexType>(pointTy);

                return isSignlessInteger || complexPt ? success() : emitOpError("argument point must be a 32-bit "
                                                                                "integer, or a complex number");
            }

            // Rewrites (x*x - y*y) as (x+y)(x-y) if x^2 and y^2 have no other uses.
            // struct DifferenceOfSquares : public OpRewritePattern<SubOp>
            // {
            //     DifferenceOfSquares(mlir::MLIRContext *context) : OpRewritePattern<SubOp>(context, /*benefit=*/1) {}
            ...
            void
            AddOp::getCanonicalizationPatterns(::mlir::RewritePatternSet &results,
